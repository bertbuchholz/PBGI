...
public:
    ...
    */
    ...
    Data get_data(Cube_cell const& c) const;
    Data get_data(int const plane, int const x, int const y) const;
    Data get_data_interpolated(Cube_cell const& c) const;
...
public:
    ...
    void setup(int const resolution);
...
void Cube_raster_buffer<Data>::setup_surfel_buffer(int resolution)
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        buffers[i] = boost::shared_ptr<Abstract_frame_buffer<Data> >(new Simple_single_value_frame_buffer<Data>(_resolution));
    }
}
...
template <class Data>
Data Cube_raster_buffer<Data>::get_data(Cube_cell const& c) const
{
...
template <class Data>
Data Cube_raster_buffer<Data>::get_data(int const plane, int const x, int const y) const
{
    return buffers[plane]->get_color(x, y);
}
...
template <class Data>
Data Cube_raster_buffer<Data>::get_data_interpolated(Cube_cell const& c) const
{
...
class Disc_splat_strategy : public Splat_strategy
{
public:
    void splat(Splat_cube_raster_buffer & cube_buffer, Gi_point_info const& point_info);
};

class Square_splat_strategy : public Splat_strategy
{
public:
    void splat(Splat_cube_raster_buffer & cube_buffer, Gi_point_info const& point_info);
};
